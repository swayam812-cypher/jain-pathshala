...
export default function Certificate() {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => setError(err.response?.data?.error || t("unableToLoadCertificate")));
  }, [bookId]);
  ...
  if (!cert) return <div className="min-h-screen bg-cream"><Navbar /><p className="p-10 text-slate/40">{t("loading")}</p></div>;

  const issueDate = new Date(cert.issue_date).toLocaleDateString(language === "hi" ? "hi-IN" : "en-US", { year: "numeric", month: "long", day: "numeric" });
    ...
    <div className="min-h-screen bg-cream">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="flex items-center justify-between">
          <button onClick={() => navigate("/")} className="text-sm text-slate/50 hover:text-slate">{t("backToJourneyArrow")}</button>
        ...
        <div className="relative mt-8 border-2 border-maroon p-1">
          <div className="border border-maroon p-14 text-center" style={{ background: "linear-gradient(180deg,#fff 0%,#fdf6ec 100%)" }}>
            ...
            <p className="mt-3 text-[11px] font-medium tracking-[0.2em] text-slate/50">{t("appName")}</p>
            <h1 className="mt-4 font-display text-4xl text-slate">{t("certificateOfCompletion")}</h1>
            <p className="mt-5 text-sm italic text-slate/60">{t("acknowledgeThat")}</p>
            <p className="mt-3 font-display text-3xl text-maroon">{cert.full_name}</p>
            <p className="mt-4 text-sm text-slate/70">{t("hasCompletedBook")}</p>
            <p className="mt-2 font-display text-2xl text-slate">{cert.book_title}</p>

            <div className="mt-10 flex justify-between text-left">
              <div>
                <p className="text-[10px] font-medium tracking-[0.14em] text-slate/40">{t("dateOfIssue")}</p>
                <p className="mt-1 text-sm text-slate">{issueDate}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-medium tracking-[0.14em] text-slate/40">{t("serialNumber")}</p>
